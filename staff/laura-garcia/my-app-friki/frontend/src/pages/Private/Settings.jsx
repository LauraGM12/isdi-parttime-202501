import { useState, useEffect } from 'react'
import { useNavigate } from "react-router"
import Form from '../../components/lib/Form'
import logics from '../../logic'
import getToken from '../../logic/helpers/getToken'
import locales from '../../locales'
import './Settings.css'

const Settings = ({ locale }) => {
    const [translations, setTranslations] = useState(locales[locale]['settings'])
    const [showNewEmailForm, setShowNewEmailForm] = useState(false)
    const [showNewPasswordForm, setShowNewPasswordForm] = useState(false)
    const [showDeleteAccountForm, setShowDeleteAccountForm] = useState(false)
    const navigate = useNavigate()

    useEffect(() => {
        setTranslations(locales[locale]['settings'])
    }, [locale])

    const emailObject = { 
        label: translations.emailLabel, 
        inputType: 'email', 
        inputPlaceholder: translations.emailPlaceholder, 
        inputId: 'email', 
        isRequired: true 
    }
    const oldPasswordObject = { 
        label: translations.oldPasswordLabel, 
        inputType: 'password', 
        inputPlaceholder: '·········', 
        inputId: 'old-password', 
        isRequired: true 
    }
    const newPasswordObject = { 
        label: translations.newPasswordLabel, 
        inputType: 'password', 
        inputPlaceholder: '·········', 
        inputId: 'new-password', 
        isRequired: true 
    }
    const newPasswordConfirmObject = { 
        label: translations.confirmPasswordLabel, 
        inputType: 'password', 
        inputPlaceholder: '·········', 
        inputId: 'confirm-password', 
        isRequired: true 
    }
    const passwordObject = { 
        label: translations.deletePasswordLabel, 
        inputType: 'password', 
        inputPlaceholder: '·········', 
        inputId: 'password', 
        isRequired: true 
    }

    const onUpdateEmail = (formData) => {
        try {
            logics.users.updateEmail(formData.email)
            alert(translations.emailSuccess)
            setShowNewEmailForm(false)
        } catch (error) {
            alert(translations.error)
            console.error(error)
        }
    }

    const onUpdatePassword = (formData) => {
        try {
            logics.users.updatePassword(formData['new-password'], formData['confirm-password'], formData['old-password'])
            alert(translations.passwordSuccess)
            setShowNewPasswordForm(false)
        } catch (error) {
            alert(translations.error)
            console.error(error)
        }
    }

    const onDeleteAccount = (formData) => {
        try {
            const doesUserAgree = confirm(translations.confirmDelete)
            if (doesUserAgree) {
                logics.users.deleteUser(formData.password)
                logics.users.logoutUser()
                navigate('/register')
            }
        } catch (error) {
            alert(error)
            console.error(error)
        }
    }

    return (
        <div className='settings'>
            <section className='settings__section'>
                <div className='settings__section-header' onClick={() => setShowNewEmailForm(!showNewEmailForm)}>
                    <h2 className='settings__section-title'>
                        <i className="bi bi-envelope"></i>
                        {translations.updateEmail}
                    </h2>
                    <i className={`settings__section-icon bi bi-chevron-down ${showNewEmailForm ? 'settings__section-icon--open' : ''}`}></i>
                </div>
                {showNewEmailForm && (
                    <div className='settings__form-container'>
                        <Form inputsArray={[emailObject]} submitButtonText={translations.saveNewEmail} onSubmitCallback={onUpdateEmail} />
                    </div>
                )}
            </section>

            <section className='settings__section'>
                <div className='settings__section-header' onClick={() => setShowNewPasswordForm(!showNewPasswordForm)}>
                    <h2 className='settings__section-title'>
                        <i className="bi bi-key"></i>
                        {translations.updatePassword}
                    </h2>
                    <i className={`settings__section-icon bi bi-chevron-down ${showNewPasswordForm ? 'settings__section-icon--open' : ''}`}></i>
                </div>
                {showNewPasswordForm && (
                    <div className='settings__form-container'>
                        <Form 
                            inputsArray={[oldPasswordObject, newPasswordObject, newPasswordConfirmObject]} 
                            submitButtonText={translations.saveNewPassword} 
                            onSubmitCallback={onUpdatePassword} 
                        />
                    </div>
                )}
            </section>

            <section className='settings__section settings__section--danger'>
                <div className='settings__section-header' onClick={() => setShowDeleteAccountForm(!showDeleteAccountForm)}>
                    <h2 className='settings__section-title'>
                        <i className="bi bi-exclamation-triangle"></i>
                        {translations.deleteAccount}
                    </h2>
                    <i className={`settings__section-icon bi bi-chevron-down ${showDeleteAccountForm ? 'settings__section-icon--open' : ''}`}></i>
                </div>
                {showDeleteAccountForm && (
                    <div className='settings__form-container'>
                        <Form inputsArray={[passwordObject]} submitButtonText={translations.deleteMyAccount} onSubmitCallback={onDeleteAccount} />
                    </div>
                )}
            </section>
        </div>
    )
}

export default Settings